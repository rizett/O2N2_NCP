...
%
% USAGE:-------------------------------------------------------------------
% [Fd, Fc, Fp, Deq, k] = fas_L13(C,u10,S,T,slp,gas,rh,beta)
% [Fd, Fc, Fp, Deq, k] = fas_L13(0.01410,35,5,10,1,'Ar',0.9,1)
%   >Fd = -5.2641e-09
%   >Fc = 1.3605e-10
%   >Fp = -6.0093e-10
%   >Deq = 0.0014
...
% beta: scaling (decimal percent) of small / large bubble mass transfer 
%       coefficients; defalt set to 1. 
%       (e.g. Bushinsky & Emerson, 2015 use beta = 0.29; Yang et al.,
%       2017 use beta = 0.53, and Emerson et al., 2019 use beta = 0.37)
%
...
% Gsat: Gas saturation state                          (%)
% Kb:   Large bubble exchange coefficient
%
...
% Modified by Robert Izett rizett@eoas.ubc.ca (added scaling coefficient)
...
function [Fd, Fc, Fp, Deq, Ks, Gsat, Kb] = fas_L13(C,u10,SP,pt,pslp,gas,rh,beta)
% -------------------------------------------------------------------------
% Conversion factors
% -------------------------------------------------------------------------
...
% if beta is not provided, set to 1 
if ~exist('beta','var')
    beta=1;
elseif isempty(beta)
    beta=1;
...
Kb = beta.*1.98e6.*ustarw.^2.76.*(ScW./660).^(-2/3)./(m2cm.*h2s); %cm/hr --> m/s
...
% Fs in L13 eqn 3
    Fd = Ks.*Geq.*(pslpc-Gsat); % Fd >0 = flux INTO ocean
% Fp in L13 eqn 3
    Fp = Kb.*Geq.*((1+dP).*pslpc-Gsat); % Fp >0 = flux INTO ocean
% L13 eqn 15
    Fc = beta.*xG.*5.56.*ustarw.^3.86; % Fc always >= 0; flux INTO ocean
